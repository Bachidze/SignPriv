...
export class AuthService {
  ...
  async signIn(signInDto:SignInDto){
    ...
    const exsistingUser = await this.userService.findOneByEmail(signInDto.email)
    if(!exsistingUser) throw new BadRequestException("invalid credentials")
    const isPasswordEqual = await bcrypt.compare(signInDto.password,exsistingUser.password)
